...
def reduce_gens(gen_list, word_length=2):
    """
    Naive function to reduce number of generators
    """
    num_gens = len(gen_list)
    print 'Total gens = {}'.format(num_gens)
    half_gens = floor(num_gens/2)
    reduced_gen_list = gen_list[:half_gens]
    secondary_gen_list = gen_list[half_gens:]
    num_sec_gens = len(secondary_gen_list)
    print 'Secondary gens = {}'.format(len(secondary_gen_list))
    for g in reduced_gen_list:
        for h in reduced_gen_list:
            for k in reduced_gen_list:
               if g*h*k in secondary_gen_list:
                    secondary_gen_list.remove(g*h*k)

    print 'New secondary gens = {}'.format(len(secondary_gen_list))
           
    gens_removed = num_gens - num_sec_gens + len(secondary_gen_list)
    percent_removed = float(100*gens_removed/num_gens)
    print 'Percent removed = {}%'.format(100 - percent_removed)



def reduce_gens2(gen_list, word_length=2):
    """
    Naive function to reduce number of generators
    """
    print 'num gens = {}'.format(len(gen_list))
    second_gen_list = gen_list[:]
    for g in gen_list:
        for h in gen_list:
               if g*h in second_gen_list:
                    second_gen_list.remove(g*h)
                    print len(second_gen_list)

    print 'New secondary gens = {}'.format(len(second_gen_list))
           
    gens_removed = len(gens_list) - len(second_gen_list)
    percent_removed = float(100*gens_removed/len(gens_list))
    print 'Percent removed = {}%'.format(percent_removed)


def reduce_gens3(gen_list, word_length=2):
    """
    Naive function to reduce number of generators
    """
    print 'num gens = {}'.format(len(gen_list))
    ...
    while len(gen_list) > 0:
        ...
        for h in gen_list:
            ...
            if h*g in gen_list:
                gen_list.remove(h*g)
        new_gen_list.append(g)
        

    gens_removed = num_gens - len(new_gen_list)
    percent_removed = float(100*gens_removed/num_gens)
    print 'Percent removed = {}%'.format(percent_removed)
...
gens_dict,gens_list,gens_words = G.compute_generators()
reduce_gens3(gens_list)
