...
The abelian group $\Delta$ is finite, of exponent dividing $p-1$. For each character $\psi$ in $\Hom(\Delta, \Z_p^\times)$, we have its corresponding idempotent $e_\psi$ in the group ring $\Z_p[\Delta]$. These characters are often called \textit{branch characters}.  The topological $\Z_p$-algebra $e_\psi \Z_p[[\widetilde{G}]]$ is isomorphic to $\Z_p[[\widetilde{G}_\infty]]$. We have the following natural isomorphism of topological $\Z_p$-algebras:
...
\begin{conjecture}
\item $\Sha^1_{f,\psi,\chi}(\Q)^\vee$ is a pseudo-null $\Lambda$-module.
\end{conjecture}
...
\begin{align*}
L_{p,f,\psi,\chi}(x,y) \text{ and } L_{p,f,\psi,\chi}(y,x).
\end{align*}
