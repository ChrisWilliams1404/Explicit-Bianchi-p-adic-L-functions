...
\subsubsection*{Notations} We first introduce the notations required to describe our results. Let $p$ be an odd prime number. Let $K$ be an imaginary quadratic field\footnote{so $K$ equals $\Q(\sqrt{-D})$, where $D$ belongs to $\{1,2,3,7,11,19,43,67,163\}$.} with class number one, where the prime $p$ splits as $(p) = \pri \bar{\pri}$. Let $\cO_K$ denote the ring of integers of $K$. Let $K_\pri$, $K_{\overline{\pri}}$ denote the unique $\Z_p$ extensions of $K$ unramified outside $\pri$, $\overline{\pri}$ respectively. Let $G_\pri$, $G_{\overline{\pri}}$ denote $\Galois{K_\pri}{K}$, $\Galois{K_{\overline{\pri}}}{K}$ respectively. Let $\widetilde{K}$ denote the maximal abelian extension of $K$, unramified outside $p$. Let $\widetilde{K}_\infty$ denote the compositum of all the $\Z_p$-extensions of $K$. Let $\widetilde{G}$, $\widetilde{G}_\infty$ denote $\Galois{\widetilde{K}}{K}$, $\Galois{\widetilde{K}_\infty}{K}$ respectively. Let $\Delta$ denote $\ker(\widetilde{G} \twoheadrightarrow \widetilde{G}_\infty)$. Since the class number of $K$ equals one, the natural restriction maps $\widetilde{G}_\infty \twoheadrightarrow G_\pri$ and $\widetilde{G}_\infty \twoheadrightarrow G_{\overline{\pri}}$ allow us to identify $\widetilde{G}_\infty$ with  $G_\pri \times G_{\overline{\pri}}$. Recall that we have the following isomorphisms of topological groups:
\begin{align*}
\widetilde{G} \cong \Delta \times \widetilde{G}_\infty, \qquad \widetilde{G}_\infty \cong \Z_p^2, \qquad G_{\pri} \cong \Z_p, \qquad G_{\overline{\pri}} \cong \Z_p.
\end{align*}

The abelian group $\Delta$ is finite, of exponent dividing $p-1$. For each character $\chi$ in $\Hom(\Delta, \Z_p^\times)$, we have its corresponding idempotent $e_\chi$ in the group ring $\Z_p[\Delta]$. These characters are often called \textit{branch characters}.  The topological $\Z_p$-algebra $e_\chi \Z_p[[\widetilde{G}]]$ is isomorphic to $\Z_p[[\widetilde{G}_\infty]]$. We have the following natural isomorphism of topological $\Z_p$-algebras:
\begin{align}\label{eq:ringdecomp}
\Z_p[[\widetilde{G}]] &\cong \Z_p[[\widetilde{G}_\infty]][\Delta] \notag \\ & \cong \prod_{\chi \in \Hom(\Delta, \Z_p^\times)} e_\chi\Z_p[[\widetilde{G}]]
\end{align}

Henceforth, we shall fix a branch character $\chi:\Delta \rightarrow \Z_p^\times$ and simply denote the corresponding component $e_\chi\Z_p[[\widetilde{G}]]$ by $\Z_p[[\widetilde{G}_\infty]]$. Let us choose a topological generator $g_\pri$  of $G_\pri$. Under the action of complex conjugation on $g_\pri$, one obtains a topological generator $g_{\overline{\pri}}$ of  $G_{\overline{\pri}}$. This further allows to consider the following isomorphism of topological $\Z_p$-algebras:
...
Throughout this paper, we shall fix the isomorphism in (\ref{eq:iwid})  and denote the $2$-variable power series ring $\Z_p[[x,y]]$ appearing in (\ref{eq:iwid}) by $\Lambda$. Let $\psi: \Gal\left({\overline{K}}/{K}\right) \rightarrow \Z_p^\times$ be a finite continuous character with conductor prime to p. Let $\kappa: \Galois{\overline{K}}{K} \twoheadrightarrow \widetilde{G}_\infty \rightarrow \GL_1(\Lambda)$ denote the tautological character.
...
Let $\D_2(\Q_p)$ (and $\D_2(\Z_p)$) denote the space of $\Q_p$-valued (and $\Z_p$-valued) $p$-adic distributions on $\Z_p^2$, endowed with a \textit{weight two} action of $\Gamma$. Let $\Symb_\Gamma(\D_2(\Q_p))$ denote the corresponding space of overconvergent modular symbols valued in $\D_2(\Q_p)$. There is a natural action of the Hecke algebra (of weight two and level $\Gamma$) on $\Symb_\Gamma(\D_2(\Q_p))$. Since $f$ satisfies \ref{hyp:ss},  a control theorem \cite[Thm.~6.10]{Wil17} shows that there exists a non-zero element $\Phi_f$ in $\Symb_\Gamma(\D_2(\Q_p))$, unique up to a scalar in $\Q_p$, that is stable under the action of the Hecke algebra and whose Hecke eigenvalues coincide with those of $f$. Associated to the tuple $(p,f,\chi,\psi)$, there exists a (locally analytic) distribution ${\mu}_{f,\chi,\psi}$ in $\D_2(\Q_p)$, satisfying certain interpolation properties which we elucidate in Section \ref{sec:l-functions} \BP{some attribution to the predicted properties is required here. I suppose this prediction is attributed to Coates-Perrin Riou in the ordinary setting, but what about in the supersingular setting? Is there some sort of uniform prediction?}.


\subsection{Main Aim}  The starting point of our paper is a precise formula \BP{refequation} that computes \textit{moments} of the distribution ${\mu}_{f,\chi,\psi}$ in terms of the overconvergent modular symbol $\Phi_f$ (and in terms of the characters $\chi$, $\psi$). To illustrate in the introduction the main output of our algorithm and keeping in mind the applications to Iwasawa theory in Section \ref{sec:appIw}, we will place ourselves in the following simplifying situation. Let $E$ be an elliptic curve defined over $\Q$. Let $T_p(E)$ denote the $p$-adic Tate module associated to $E/\Q$. Let $f$ denote the cuspidal Bianchi eigenform corresponding to the base change to $K$ of the weight two $p$-stabilized cuspidal eigenform over $\Q$ associated to $E$. One can consider a $4$-dimensional Galois representation $\tau:G_\Q \rightarrow \GL_4(\Lambda)$, given by the action of $\Galois{\overline{\Q}}{\Q}$ on $\Ind^\Q_K \left( \Res^\Q_K T_p(E) \otimes_{\Z_p} \Lambda(\kappa^{-1})\right)$ --- a free $\Lambda$-module of rank $4$. \\


\subsubsection*{When $E$ has good ordinary or split multiplicative reduction at $p$} In this case, it is known that ${\mu}_{f,\chi,\psi}$ belongs to $\D_2(\Z_p)$. Recall that the Amice transform provides an  isomorphism $\D_2(\Z_p) \cong \Lambda$ of topological $\Z_p$-algebras. The purpose of the algorithm in our paper in this case is to compute the Amice transform, denoted $L_{p,f,\chi,\psi}(x,y)$, of $\mu_{f,\chi,\psi}$. To wit, we have
\begin{align*}
\D_2(\Z_p) &\cong \Lambda, \\
\mu_{f,\chi,\psi} &\leftrightarrow L_{p,f,\chi,\psi}(x,y).
\end{align*}
Corresponding to the fact that $E$ has good ordinary or split multiplicative reduction at $p$, one observes that the Galois representation $\tau$ satisfies Greenberg's Panchishkin condition\footnote{In fact, $\tau$ satisfies another distinct Panchishkin condition since $p$ splits in $K$.} and one can formulate an \textit{Iwasawa-Greenberg main conjecture} associated to $\tau$. See \cite{MR1265554} for more details. The application to Iwasawa theory stems from the fact that it is $L_{p,f,\chi,\psi}(x,y)$ that appears in the formulation of this conjecture, and is conjectured to generate the characteristic divisor in $\Lambda$ of the Pontryagin dual of a Selmer group (which we denote $\Sel_{f,\chi,\psi}(\Q)^\vee$).   \\

\subsubsection*{When $E$ is good supersingular at $p$ with $a_p(E)=0$} In this case, the distribution $\mu_{f,\chi,\psi}$ does not belong to $\D_2(\Z_p)$. \CWnote{Actually, the $p$-adic $L$-function is in $\D_2$! This is the space of distributions, not measures. It is not, however, a measure, that is a bounded distribution/distribution of growth 0. We should perhaps denote the subspace of measures by $\D_2^0$?} However, by suitably modifying this distribution (following an approach of Pollack \cite{MR1983040} in the $1$-variable case over $\Q$), Loeffler \cite{Loe14} has constructed four distributions $\mu^{\pm,\pm}_{f,\chi,\psi}$ in $\D_2(\Z_p)$. These distributions are referred to as \textit{doubly-signed} $p$-adic $L$-functions.  The purpose of the algorithm in our paper in this case is to compute the Amice transform, denoted $L^{\pm,\pm}_{p,f,\chi,\psi}(x,y)$, of $\mu^{\pm,\pm}_{f,\chi,\psi}$. To wit, we have
\begin{align*}
\D_2(\Z_p) &\cong \Lambda, \\
\mu^{\pm,\pm}_{f,\chi,\psi} &\leftrightarrow L^{\pm,\pm}_{p,f,\chi,\psi}(x,y).
\end{align*}
The application to Iwasawa theory in this setting stems from conjectures of Kim \cite{MR3224266}. Each of the doubly signed $p$-adic $L$-functions $L^{\pm,\pm}_{p,f,\chi,\psi}(x,y)$ is conjectured to generate the characteristic divisor in $\Lambda$ of the corresponding doubly signed Selmer group (which we denote $\Sel^{\pm,\pm}_{f,\chi,\psi}(\Q)^\vee$) constructed by Kim \cite{MR3224266}, which in turn is inspired by work over $\Q$ of Kobayashi \cite{MR1965358}.
...
Since $f$ satisfies \ref{hyp:ss},  the control theorem \cite[Thm.~6.10]{Wil17} is also applicable for the parabolic cohomology groups. There exists a unique cohomology class $[\Theta_{f}]$ in $H^1_{\Par}\left(\Gamma,\D_2(\Q_p) \right)$, stable under the action of the Hecke algebra, such that $\rho([\Theta_{f}])$ equals $[\vartheta_{f}]$. In \ref{step2}, we construct a cocycle $\Theta$ in $Z^1(\Gamma,\D_2(\Q_p))$ such that its corresponding cohomology class in $H^1_{\Par}\left(\Gamma,\D_2(\Q_p) \right)$ equals $[\Theta_{f}]$.
...
\item\label{step4} \underline{\emph{Computing the moments of the distribution $\mu_{f,\chi,\psi}$}}.
\end{enumerate}
...
\subsection{Applications in Iwasawa theory} \label{sec:appIw}

\BP{Oops - sorry, woefully incomplete. To do:
\begin{enumerate}
\item Figure out how the action of complex conjugation works. Is it enough to check that  $L_p(x,y)$ and $L_p(y,x)$ are co-prime? Do you really need that the elliptic curves in this case are CM? 
\end{enumerate}
}


\subsubsection{Higher codimension Iwasawa theory}
Throughout Section \ref{sec:appIw}, suppose as before that $f$ equals the cuspidal Bianchi eigenform corresponding to the base change to $K$ of a weight two $p$-stabilized cuspidal eigenform over $\Q$ associated to an elliptic curve $E$ over $\Q$. Our application involve supporting a conjecture Coates-Sujatha \cite[Conjecture B]{MR2148798} on the Pontryagin dual of the fine Selmer group (which we denote $\Sha^1_{f,\chi,\psi}(\Q)^\vee$). The discrete fine Selmer group associated to $\tau$ is, roughly speaking, the subgroup of the first global Galois cohomology group that is locally trivial at all primes. The philosophy behind this conjecture of Coates-Sujatha goes back to conjectures of Greenberg \cite{Gre76}, \cite[Conjecture 3.5]{MR1846466}.

A finitely generated torsion $\Lambda$-module is said to be \textit{pseudo-null} if its characteristic divisor $\mathrm{Div}(M)$ in $\Lambda$ equals zero. The conjecture of Coates-Sujatha in our setting amounts to the following:

\begin{conjecture}
\item $\Sha^1_{f,\chi,\psi}(\Q)^\vee$ is a pseudo-null $\Lambda$-module.
\end{conjecture}



\subsubsection*{Target examples}
Tables \ref{good/ordinary} and \ref{supersingular} provide a list of target examples where our computations can be used to prove pseudo-nullity.

Our methodology in the good ordinary case (Table \ref{good/ordinary}) will be to show that there exists no irreducible element in $\Lambda$ that divides both
\begin{align*}
L_{p,f,\chi,\psi}(x,y) \text{ and } L_{p,f,\chi,\psi}(y,x).
\end{align*}


\BP{Following examples should be interesting. Need to actually verify that they will be interesting and then explain why}



\begin{table}	\caption{\label{good/ordinary}\emph{Examples of CM $E/\Q$ with good ordinary reduction at $p$}}
...
\begin{table}	\caption{\label{supersingular}\emph{Examples of $E/\Q$ with good supersingular reduction at $p$}}
...
\BP{this is just an additional comment/observation, not related to higher codimension Iwasawa theory but might be interesting nonetheless --- will work on this.}

Then, the Galois representation $\tau$ is isomorphic to
\begin{align*}
T_p(E) \otimes_{\Lambda} \Ind^\Q_K (\Z_p)  \Lambda(\kappa_{\pri}^{-1}) \otimes_\Lambda \Lambda(\kappa_{\mathrm{cyc}}^{-1}).
\end{align*}
This corresponds to the tensor product of the elliptic curve $E$ and the ``CM-Hida family'' corresponding to the
Hida has constructed a $2$-variable Rankin-Selberg $p$-adic $L$-function.  where the elliptic curve $E$ has the dominant weight (equal to two) and  we vary over the classical weight one specializations of the induced represnetat the cyclotomic variable and the weight variable (of the CM family).  For this Rankin-Selberg $p$-adic $L$-function, one can vary the cyclotomic variable and the weight-variable.
...
In the computations, we restrict to imaginary quadratic fields of class number one for simplicity. In this case, the space of modular symbols is rich enough to capture all of the arithmetic of the form $f$. In the case of class number $h$, we would instead need to consider a direct sum of $h$ copies of modular symbols, with Hecke operators at non-principal primes permuting the factors; see, for example, \cite{Wil17} for a detailed description of this. Such a decomposition arises since the Bianchi three-fold in this situation decomposes into a disjoint union of $h$ connected components via strong approximation for $\GLt/K$.
...
\BP{
\section*{Bookkeeping:}
\begin{enumerate}\item I chose $f$ to represent the Bianchi modular form instead of the calligraphic $\f$.
\item I chose to represent the modular symbols by $\Phi_f$ and $\varphi_f$. I chose to represent the objects in parabolic cohomology by $[\Theta_f]$ and $[\vartheta_f]$. Any cocycle corresponding to these cohomology classes are represented without the brackets by $\Theta$ and $\vartheta$. In particular, also notice that the overconvergent objects are represented by an uppercase Greek alphabet $\Phi$ and $[\Theta]$, whereas the objects with trivial coefficients are lowercase ``var'' Greek $\varphi$ and $[\vartheta]$.
\item I am starting to suspect that the the constant term of the 2-variable power series is zero, if the Elliptic curve has rank 2 over K --- can this be seen from the analytic side?
\item If I wanted to remove some text from the pdf, I placed stuff under the comment environment in the tex file.
\item My 2 cents --- for the purposes of narrative flow, I think it may be better to have in an appendix rather than the main text --- the theory of modular symbols over $\Q$, along with description of the main conjectures and explanations of why the examples we work out are interesting.
\end{enumerate}
}
