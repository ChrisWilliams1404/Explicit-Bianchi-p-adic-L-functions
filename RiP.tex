...
\begin{align}\label{eq:modular symbol}
    \phi_{\f} : \Delta_0 &\longrightarrow \C,\notag\\
    \{r\to s\} &\longmapsto \int_{r}^s \delta_{\f},
\end{align}
...
\section{Explicit values of modular symbols}
We give explicit formulae for the values of $\phi_{\f}$ at specific paths $\{r \to \infty\}$, for $r \in K$. One may take a vertical path in $\uhp_3$, with no variation in the $z,\overline{z}$ terms; in particular, in this path $z = r$ is fixed and $t$ ranges from $0$ to $\infty$. Then \eqref{eq:modular symbol} collapses to
	\[
		\phi_{\f}\{r \to \infty\} = -\int_{0}^\infty \f_1(r,t) dt.
	\]
	As in \cite[(2.1)]{CrWh94}, we have a Fourier expansion 
	\[
		\f_1(z,t) = \sum_{\alpha \in \cO_K^\times} c(\alpha,\f) K_0\left(\frac{4 \pi |\alpha| t}{\sqrt{|D|}}\right) e^{\frac{2\pi i (\alpha z - \overline{\alpha z})}{\sqrt{-D}}},
	\]
	where $K_0$ is the $K$-Bessel function defined e.g.\ in \cite[\S1.2.1]{Wil17}. (Note this Fourier expansion is normalised slightly differently from that in \cite[(1.3)]{Wil17}). We have the following standard integral:
	\[
		\int_0^\infty K_0(\lambda t)dt = (2\lambda)^{-1} \Gamma(1/2)^2 = \frac{\pi}{2\lambda}.
	\]
	In particular, we get
	\[
		\int_0^\infty K_0\left(\frac{4\pi |\alpha|t}{\sqrt{|D|}}\right) dt = \frac{\sqrt{|D|}}{8|\alpha|}.
	\]
	\CWnote{Now need the break point to ensure convergence, and Atkin--Lehner. Note Cremona Whitley do something similar in section 2.6 of their paper. I suspect there might be a subtlety though when we want to go from $r$ to $\infty$ rather than $0$: it surely won't be fixed by Atkin-Lehner any more? But if we move off the vertical line then the integral will become terrible (including other Fourier expansions too).
		
		 How does Christian's method deal with dividing by periods? CW have something on computing this as well.}
