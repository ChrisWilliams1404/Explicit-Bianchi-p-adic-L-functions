...
class Sigma0Squared(Parent,UniqueRepresentation):
        ...
        def _an_element_(self):
                return Sigma0SquaredElement(self,(self._Sigma0([1,0,0,1]), self._Sigma0([1,0,0,-1])))
        ...
        def __repr__(self):
                return 'Monoid Sigma_0(%s) x Sigma_0(%s) with coefficients in %s' % (self.level(),self.level(), self.base_ring())
...
class Sigma0SquaredAction(Action):
        ...
        def _call_(self,mu,g):
                ...
                acted_moments = (matrix_of_action * vector_mu).change_ring(mu.parent().base_ring())
...
class BianchiDistributionElement(ModuleElement):
    ...
    def max_filtration_step(self):
        ...
        min_modified_moment = min((o[0].valuation() + sum(tuple(self.parent().ij_from_pos(n))) for n,o in enumerate(self._moments)))
...
class BianchiDistributions(Module,UniqueRepresentation):
    ...
    def __init__(self,p,depth, act_on_left=False, adjuster=None):
        ...
        self._repr_R = PowerSeriesRing(self._R, num_gens=2, default_prec=self._depth, names='X,Y')

        self._Sigma0Squared = Sigma0Squared(self._p, self._Rmod, adjuster)
        self._act = Sigma0SquaredAction(self._Sigma0Squared,self, act_on_left=act_on_left)
        ...
        self._PowerSeries_x = PowerSeriesRing(self._Rmod, default_prec = self._depth, names='x')
        self._PowerSeries_x_ZZ = PowerSeriesRing(ZZ, default_prec = self._depth, names='x')
        self._PowerSeries = PowerSeriesRing(self._PowerSeries_x, default_prec = self._depth, names='y')
        self._PowerSeries_ZZ = PowerSeriesRing(self._PowerSeries_x_ZZ, default_prec = self._depth, names='y')
    ...
    def _an_element_(self):
        r"""
        """
        return BianchiDistributionElement(self,Matrix(self._R,self._dimension,1,range(1,self._dimension+1)), check = False)
    ...
    def _repr_(self):
        ...
        """
        return "Space of %s-adic Bianchi distributions with k=0 action and precision cap %s"%(self._p, self._dimension - 1)
    ...
    def basis(self):
        ...
        self._basis=[BianchiDistributionElement(self,Matrix(self._R,self._dimension,1,{(jj,0):1},sparse=False),check = False) for jj in range(self._dimension)]
        return self._basis
